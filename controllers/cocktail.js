...
exports.addCocktail = async (req, res) => {
  const { userId, name, description, recipe } = req.body;

  // Validation des données reçues
  if (!userId || !name || !description || !recipe) {
    return res.status(400).json({ message: "Missing Data" });
  }

  try {
    // Vérification si le coktail existe
    let cocktail = await Cocktail.findOne({ where: { name: name }, raw: true });
    if (cocktail !== null) {
      return res
        .status(409)
        .json({ message: `The cocktail ${name} already exists !` });
    }
